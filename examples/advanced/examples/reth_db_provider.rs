...
//         // This would actually use an existing reth datadir specified by `--datadir` when
// starting         // your reth node.
//         let reth = Reth::new()
//             .dev()
//             .disable_discovery()
//             .block_time("1s")
...
//         // Any RPC method that is not implemented in the RethDbProvider gracefully falls back to
// the         // RPC provider specified in the `connect_http` method.
...
//             let provider =
// this.provider_at(block_id).map_err(TransportErrorKind::custom).unwrap();
